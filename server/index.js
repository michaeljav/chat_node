...
io.on('connection', async (socket) => {
  ...
  socket.on('chat message', async (msg) => {
    ...
    //emit to all clients
    io.emit('chat message', msg, result.lastInsertRowid.toString(), username); //enviamos el mensaje a todos los clientes conectados
  ...
  if (!socket.recovered) {
    ...
    try {
      ...
      result.rows.forEach((row) => {
        socket.emit('chat message', row.content, row.id.toString(), row.user); //enviamos el mensaje a todos los clientes conectados
      });
    } catch (error) {
